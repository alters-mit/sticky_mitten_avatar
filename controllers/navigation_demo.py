...
class NavigationDemo(DemoController):
    ...
    def _put_object_in_container_by_index(self, object_index: int, container_index: int) -> None:
        ...
        self.go_to(target=self._target_object_ids[object_index], move_stopping_threshold=0.8)
        self._lift_arm(arm=self._container_arm)
        self._go_to_and_lift(object_id=self._target_object_ids[object_index],
                             arm=self._object_arm, stopping_distance=0.4)
